<template>
  <div class="success-container">
    ...
    <!-- Customize tickets button (if applicable) -->
    <div v-if="orderLoaded && orderHasTickets" class="customize-section">
      <button class="customize-button">Customize My Tickets</button>
    </div>

    <NuxtLink class="back-home" to="/">Back to Home</NuxtLink>
  </div>
</template>
...
// Retrieve the provider_order_id from the URL
const providerOrderId = route.query.provider_order_id

const orderItems = ref([])
const orderTotal = ref('0.00')
const orderHasTickets = ref(false)
const ticketFees = ref(null)
...
async function fetchOrderSummary() {
  try {
    if (providerOrderId && typeof providerOrderId === 'string') {
      ...
      // ticketFees is calculated on the backend (3.5% + 0.50€ on the total ticket amount)
